...
contract MyToken is ERC20, Ownable {
    ...
    // mapping delegator to its sole delegatee
    mapping(address => address) public delegateHistory;
    ...
    function revokeDelegate() external {
        uint256 senderBalance = balanceOf(msg.sender);
        require(senderBalance > 0, "Delegtor has no balance");

        // The token voting power is reset to its balance
        address delegatee = delegateHistory[msg.sender];
        votingPower[delegatee] = 0;
    ...
    function deductVotingPower(address voter, uint amount) public {
        require(votingPower[voter] >= amount);
